...
local function onSecond()
 ...
 updateFront() scanOwnSquads() detectLosses() processUnmatched() unlockAttack() processPointStart() neutralTransition() completeCaptured() processRetakes() startReady() processWaves() maintenance() processOrders() processSupport() processPatrols() scanEnemy() N.processSpawn()
...
function onGameStart()
 ...
 local host=tonumber(BotApi.Instance.hostId) or 1 math.randomseed(os.time()*host) local mapOk=N.MAP and N.MAP:load() or false queueOpening() N.log('START v1.20 map='..tostring(mapOk)) N.log('RULE BTG ASSEMBLY->APPROACH->ASSAULT; FAILED=>REGROUP+NEW ROUTE') N.log('RULE OWN SCENE SCAN=ON; SHARED RANDOM CORRIDOR=ON; ENEMY DIAGNOSTICS=ON') startClock() N.processSpawn()
end
...
function onGameSpawn(args)
 ...
 elseif ticket.role=='aircraftlight' then C.NextAircraftAllowedAt=math.max(C.NextAircraftAllowedAt or 0,C.Time+math.max(360,NEU_BOT.AirSupportCooldownSec or 360)) N.log('AIR ARRIVED cooldownUntil='..C.NextAircraftAllowedAt) local g=ticket.groupId and C.Groups[ticket.groupId] local t=(g and g.target) or ticket.target C.AirSupport[sid]={groupId=ticket.groupId,target=t,home=N.chooseOwnFlagBefore(t),leg='target'} if t then N.capture(sid,t,true) end
