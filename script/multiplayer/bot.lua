-- NEU BOT v1.13
-- Based on v1.12 hotfix; optimized scheduling and orders.
-- Fixes pointstart opening coverage, adopts infantry that dismounts from APCs into the active assault group,
-- and adds one opening helicopter that patrols the enemy player's captured flags.
...
local function processDeferredOrders()
 for i=#C.DeferredOrders,1,-1 do
  ...
  if o.due<=C.Time then
   local gid=C.SquadGroup[o.squad] local g=gid and C.Groups[gid]
   if g and not g.stopped and g.phase=="attack" and g.target then capture(o.squad,g.target) end
...
local function criticalRole(role) return role=="tank" or role=="tank80plus" or role=="infantry" or role=="patrol_heli" end
local function processSpawn()
 ...
 if ok or C.AwaitingArrival~=ticket then
  ...
  elseif intent.role=="pointstart" then
...
local function armDetachWindow(id,target,reason)
 ...
 local previous=C.DetachWindows[id]
 if previous and previous.target==target then previous.expiresAt=C.Time+(NEU_BOT.DetachedAdoptWindowSec or 25) return end
...
local function neutralTransition()
 ...
 if strategic==0 then
  ...
  if C.AttackUnlocked and not C.AttackGateLogged then C.AttackGateLogged=true log("ATTACK GATE OPEN strategicNeutral=0 hostileNeutral="..hostile.." reason="..tostring(C.AttackUnlockReason)) end
 else
  C.AttackGateLogged=false
...
local function processHeliPatrol()
 ...
 local f=flags() if #f.enemy==0 then return end
 local list={} for _,n in ipairs(f.enemy) do list[#list+1]=n end
 geoSortSpawn(list,"heli-enemy-patrol")
 local target=nil
 ...
 if target then C.PatrolHeliLastFlag=target log("HELI PATROL squad="..sid.." flag="..target.." enemyFlags="..#list) capture(sid,target,true) end
end
...
local function onSecond()
 ...
 C.FlagSnapshot=flags() C.FlagIndex={}
...
function onGameStart()
 C.Spawning=false C.FlagSnapshot=nil C.FlagIndex=nil C.AttackGateLogged=false
 ...
 log("START v1.13 army="..tostring(BotApi.Instance.army).." team="..tostring(BotApi.Instance.team).." units="..tostring((C.Units.count or 1)-1))
...
function onGameSpawn(args)
 ...
 if not sid then log("SPAWN EVENT INVALID missing squadId") return end
 if C.SquadRole[sid] then return end
