...
function N.buildCandidates()
 local roles={'pointstart','patrol_heli','recon','infantry','defense_infantry','antiair','tank','aircraftlight','artsupport','aat','antirad','strike'} C.Candidates={}
...
function N.capture(id,flag,force)
 ...
 if neutral and role~='pointstart' and not C.HostileNeutral[flag] then return end local last=C.LastOrder[id] if not force and last and last.flag==flag and C.Time-(last.time or 0)<(NEU_BOT.OrderCooldownSec or 10) then return end C.LastOrder[id]={flag=flag,time=C.Time} N.log('ORDER squad='..id..' group='..tostring(C.SquadGroup[id])..' flag='..flag..' role='..tostring(role)) BotApi.Commands:CaptureFlag(id,flag)
end
function N.enqueueSpawn(role,reason,delay,target,gid,mode) C.SpawnIntents[#C.SpawnIntents+1]={role=role,reason=reason or '',due=C.Time+(delay or 0),target=target,groupId=gid,mode=mode,attempts=0,cycles=0,tried={}} if role=='pointstart' and target then C.PointStartPending[target]=true end N.log('QUEUE role='..role..' group='..tostring(gid)..' target='..tostring(target)..' reason='..tostring(reason)) end
...
function N.processSpawn()
 ...
 local ticket={role=intent.role,target=intent.target,groupId=intent.groupId,mode=intent.mode,unit=rec.unit} C.SpawnTickets[#C.SpawnTickets+1]=ticket C.AwaitingArrival=ticket C.Spawning=true local ok=BotApi.Commands:Spawn(rec.unit,(intent.role=='pointstart') and (NEU_BOT.PointStartSquadSize or MaxSquadSize) or MaxSquadSize) C.Spawning=false
 if ok or C.AwaitingArrival~=ticket then N.log('SPAWN OK role='..intent.role..' unit='..rec.unit) else C.AwaitingArrival=nil table.remove(C.SpawnTickets,#C.SpawnTickets) local retryMax=(intent.role=='aircraftlight' or intent.role=='artsupport') and (NEU_BOT.SupportSpawnRetryCycles or 8) or (NEU_BOT.CriticalSpawnRetryCycles or 3) if intent.attempts<#list then intent.due=C.Time+(NEU_BOT.SpawnRetrySec or 2) C.SpawnIntents[#C.SpawnIntents+1]=intent elseif (intent.cycles or 0)<retryMax then intent.cycles=(intent.cycles or 0)+1 intent.attempts=0 intent.tried={} intent.due=C.Time+((intent.role=='aircraftlight' or intent.role=='artsupport') and (NEU_BOT.SupportSpawnRetrySec or 15) or (NEU_BOT.CriticalSpawnRetrySec or 10)) C.SpawnIntents[#C.SpawnIntents+1]=intent else N.log('SPAWN FAILED role='..intent.role..' group='..tostring(intent.groupId)) end end
end
return N
