-- NEU BOT v1.9
-- Based on v1.8. Fixes map JSON BOM parsing integration, opening recovery,
-- critical spawn retry, neutral-capture stalls, command spam and USA special-air roles.
...
local C={
 ...
 NextTankReinforcementAt=0,NextInfantryReinforcementAt=0,NextPatrolAt=0,NextOrderAt=0,NextNeutralAt=0,NextOpeningMaintenanceAt=0,
 NeutralCaptureTarget={},NeutralAssignedAt={},NeutralBlockedUntil={},NeutralPending={},ReconSquad=nil,ReconTarget=nil,CaptureSource="all infantry fallback",
...
local function roleMatches(r,role)
 ...
 -- Explicit v1.9 special-air exceptions. These multiplayer records are marked nobot
 -- in NEU, but they are the real units needed by logic-editor special branches.
...
local function buildCandidates()
 ...
 local preferred={}
 for _,rec in ipairs(C.Candidates.infantry or {}) do local text=recText(rec) if text:find("vehicle_supporter",1,true) then preferred[#preferred+1]=rec end end
...
local function processSpawn()
 ...
 if ok then C.AwaitingArrival=ticket log("SPAWN OK role="..intent.role.." unit="..rec.unit.." group="..tostring(intent.groupId).." maxSquad="..tostring(maxSize))
  ...
  if intent.attempts<total then intent.due=C.Time+(NEU_BOT.SpawnRetrySec or 2) C.SpawnIntents[#C.SpawnIntents+1]=intent
  ...
  else if intent.role=="capture_infantry" and intent.target then C.NeutralPending[intent.target]=nil end log("SPAWN FAILED role="..intent.role) end
 end
end
...
local function neutralBlocked(name) return (C.NeutralBlockedUntil[name] or 0)>C.Time end
...
local function chooseNeutral(used,reason)
 local f=flags() local list={} for _,n in ipairs(f.neutral) do if not (used and used[n]) and not neutralBlocked(n) then list[#list+1]=n end end geoSortSpawn(list,reason) return list[1]
end
local function processRecon()
 ...
 C.ReconTarget=nil local target=chooseNeutral(neutralUsed(),"recon-nearest-spawn") if target then C.ReconTarget=target log("RECON TARGET GEO flag="..target.." dSpawn="..tostring(MAP and MAP:distanceFromSpawn(target))) capture(C.ReconSquad,target,true) end
end
local function processNeutralCapture()
 ...
 for sid,target in pairs(C.NeutralCaptureTarget) do
  if not squadAlive(sid) then C.NeutralCaptureTarget[sid]=nil C.NeutralAssignedAt[sid]=nil if target then C.NeutralPending[target]=nil end
  elseif target and neutral[target] then
   local age=C.Time-(C.NeutralAssignedAt[sid] or C.Time)
   if age>=(NEU_BOT.NeutralCaptureStallSec or 60) then
    C.NeutralBlockedUntil[target]=C.Time+(NEU_BOT.NeutralCaptureBlockedSec or 30)
    log("NEUTRAL STALL squad="..sid.." flag="..target.." age="..age.." blockedUntil="..C.NeutralBlockedUntil[target])
    C.NeutralCaptureTarget[sid]=nil C.NeutralAssignedAt[sid]=nil
    local used=neutralUsed() local alt=chooseNeutral(used,"stalled-pair-reassign")
    if alt then C.NeutralCaptureTarget[sid]=alt C.NeutralAssignedAt[sid]=C.Time log("NEUTRAL REASSIGN squad="..sid.." from="..target.." to="..alt) capture(sid,alt,true)
    else C.NeutralCaptureTarget[sid]=target C.NeutralAssignedAt[sid]=C.Time capture(sid,target,true) end
   else capture(sid,target) end
  else
   if target then log("NEUTRAL CAPTURE DONE squad="..sid.." flag="..target.." occupant="..tostring(flagOccupant(target))) end C.NeutralCaptureTarget[sid]=nil C.NeutralAssignedAt[sid]=nil if target then C.NeutralPending[target]=nil end
  end
 end
 if f.neutralCount==0 then return end local limit=NEU_BOT.NeutralCaptureMaxActive or 3 local used=neutralUsed() local list={} for _,n in ipairs(f.neutral) do if not used[n] and not neutralBlocked(n) then list[#list+1]=n end end geoSortSpawn(list,"capture-pairs-nearest-spawn")
...
local function neutralTransition()
 local f=flags()
 if f.neutralCount==0 then if not C.NeutralsCleared then C.NeutralsCleared=true log("NEUTRALS CLEARED time="..C.Time) for id,g in pairs(C.Groups) do if not g.stopped then g.phase="ready" g.target=nil g.resultCheckAt=nil g.nextAttackAt=C.Time log("GROUP READY id="..id.." reason=neutrals-cleared") end end end
...
local function recoverDirections()
 ...
 for id,g in pairs(C.Groups) do if g.stopped and g.restartAt and C.Time>=g.restartAt then g.stopped=false g.phase="ready" g.target=nil g.nextAttackAt=C.Time+5 g.restartAt=nil g.infantry={} log("DIRECTION REBUILD group="..id.." failed="..tostring(g.failedTarget).." freshInfantry=YES freshTank=YES") enqueueSpawn("infantry","direction rebuild infantry",0,nil,id,"rebuild") enqueueSpawn("tank80plus","direction rebuild tank",0,nil,id,"rebuild") end end
end
local function detectLosses()
 for sid,role in pairs(C.SquadRole) do if not C.DeadSquads[sid] and not squadAlive(sid) then
  C.DeadSquads[sid]=true C.LastOrder[sid]=nil local id=C.SquadGroup[sid] local g=id and C.Groups[id] log("LOST squad="..sid.." role="..role.." group="..tostring(id))
  if role=="capture_infantry" then local t=C.NeutralCaptureTarget[sid] C.NeutralCaptureTarget[sid]=nil C.NeutralAssignedAt[sid]=nil if t then C.NeutralPending[t]=nil end C.NextNeutralAt=0 end
...
local function processBalanceDefense()
 ...
 if should and not C.BalanceDefenseActive then C.BalanceDefenseActive=true local used={} log("BALANCE DEFENSE ON mine="..f.mineCount.." enemy="..f.enemyCount) for _,id in ipairs(activeGroups()) do local t=chooseFrontOwnFlag(used) if t then used[t]=true enqueueSpawn("defense_infantry","geo frontline defense",0,t,id,"defense") end end elseif not should and C.BalanceDefenseActive then C.BalanceDefenseActive=false log("BALANCE DEFENSE OFF mine="..f.mineCount.." enemy="..f.enemyCount) end
end
...
local function onSecond()
 C.Time=C.Time+1 detectLosses() processDeferredOrders() unlockAttack() processEnemyRules() processRecon() processNeutralCapture() neutralTransition() processOpeningMaintenance() recoverDirections() startReadyGroups() resolveAttackResults() processOrders() processAA() processBalanceDefense() processReinforcements() processSpawn()
...
function onGameStart()
 ...
 C.AttackUnlocked=false C.AttackUnlockReason=nil C.NeutralsCleared=false C.NextTankReinforcementAt=NEU_BOT.TankReinforcementSec C.NextInfantryReinforcementAt=NEU_BOT.InfantryReinforcementSec C.NextPatrolAt=0 C.NextOrderAt=0 C.NextNeutralAt=0 C.NextOpeningMaintenanceAt=0
 C.NeutralCaptureTarget={} C.NeutralAssignedAt={} C.NeutralBlockedUntil={} C.NeutralPending={} C.ReconSquad=nil C.ReconTarget=nil C.BalanceDefenseActive=false
 ...
 log("START v1.9 army="..tostring(BotApi.Instance.army).." team="..tostring(BotApi.Instance.team).." units="..tostring((C.Units.count or 1)-1))
 log("RULE logic-editor=neutral-first two-BTG=ON recon-capture=ON capture-pair=vehicle_supporter/2")
 log("RULE opening-recovery=ON critical-spawn-retry=ON neutral-stall-reassign=ON order-cooldown="..tostring(NEU_BOT.OrderCooldownSec))
 log("RULE USA-special-air-nobot-whitelist=ON")
...
function onGameSpawn(args)
 ...
 elseif ticket.role=="capture_infantry" then C.NeutralPending[ticket.target]=nil C.NeutralCaptureTarget[sid]=ticket.target C.NeutralAssignedAt[sid]=C.Time if ticket.target then capture(sid,ticket.target,true) end
 elseif ticket.role=="infantry" then local id=ticket.groupId if id and C.Groups[id] then C.Groups[id].infantry[#C.Groups[id].infantry+1]=sid if C.Groups[id].phase=="attack" and C.Groups[id].target then capture(sid,C.Groups[id].target,true) end end
 ...
 elseif ticket.target then local o=flagOccupant(ticket.target) if o==BotApi.Instance.enemyTeam or o==BotApi.Instance.team then capture(sid,ticket.target,true) end end
 processSpawn()
end
