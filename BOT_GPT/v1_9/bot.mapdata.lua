-- NEU BOT v1.9 map knowledge
-- Reads _flag_points_final.json and optional _map_points_index.json.
-- v1.9 fixes UTF-8 BOM/leading-whitespace parsing from v1.8.
...
local function normalizeJson(text)
    ...
    -- UTF-8 BOM observed in generated scanner JSON files.
...
function M:load()
    ...
    if rawIndex then
        ...
        if ok2 and type(indexRoot)=="table" then
            ...
            log("INDEX PARSE FAILED file="..tostring(indexPath).." err="..tostring(indexRoot))
        end
    else
        log("INDEX MISSING file="..indexName)
    end
    ...
    self.loaded=true
    local count=0 for _ in pairs(self.flags) do count=count+1 end
    log("MATCH key="..self.mapKey.." runtimeFlags="..tostring(#names).." geoFlags="..tostring(count).." source="..tostring(path))
    if self.spawn then
