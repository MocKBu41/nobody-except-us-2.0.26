-- NEU BOT v1.5.2
-- Based directly on working v1.5/v1.5.1.
-- Fixes: strict neutral-first, only 2-man infantry capture squads may take neutral flags,
-- continuous neutral capture replenishment, and periodic infantry reinforcement after 5 minutes.
...
local C={
    ...
    NeutralCaptureTarget={},NeutralPending={},
...
local function availableTargets(includeEnemy)
    ...
    if #list==0 and includeEnemy then for _,n in ipairs(f.enemy) do list[#list+1]=n end end
    sortByOwnSide(list)
    return list
end
...
local function newGroup(target)
    ...
    C.Groups[id]={id=id,target=target,phase="opening",captured=false,stopped=false,
...
local function assignTankGroup()
    ...
    if not best then for gid,g in pairs(C.Groups) do if not g.stopped then best=gid break end end end
    return best
end
...
local function currentUsedTargets(exceptGid)
    ...
    for sid,target in pairs(C.DetachedTarget) do if squadAlive(sid) and target then used[target]=true end end
    return used
end
...
local function startGroupAttack(gid)
    ...
    if f.neutralCount>0 then
        g.phase="wait_neutral"
        g.nextAttackAt=nil
        log("ATTACK HOLD group="..gid.." neutralLeft="..f.neutralCount)
        return
    end
...
local function unlockAttackIfNeeded()
    ...
    if C.Time>=NEU_BOT.AttackWaitSec then
        C.AttackUnlocked=true C.AttackUnlockReason="5 minutes"
    elseif f.enemyCount>f.mineCount then
        C.AttackUnlocked=true C.AttackUnlockReason="enemy flag advantage"
    end
    if C.AttackUnlocked then
        log("ATTACK UNLOCK reason="..C.AttackUnlockReason.." time="..C.Time.." mine="..f.mineCount.." enemy="..f.enemyCount.." neutral="..f.neutralCount)
        for gid,g in pairs(C.Groups) do
            if not g.stopped and g.phase=="opening" and g.target and flagOccupant(g.target)==BotApi.Instance.team then
                g.phase="defend" g.nextAttackAt=C.Time
            end
        end
    end
end
...
local function processNeutralCapture()
    ...
    for _,x in ipairs(C.SpawnIntents) do if x.role=="capture_infantry" and x.target then used[x.target]=true end end
    ...
    for sid,target in pairs(C.DetachedTarget) do
        if squadAlive(sid) then
            if target and flagOccupant(target)==BotApi.Instance.team then
                ...
                C.DetachedTarget[sid]=nextTarget target=nextTarget
            end
...
local function processGroupOrders()
    ...
    local f=flags()
    for gid,g in pairs(C.Groups) do
        if f.neutralCount>0 and g.phase=="attack" then
            log("ATTACK PAUSE group="..gid.." enemyFlag="..tostring(g.target).." neutralLeft="..f.neutralCount)
            g.phase="wait_neutral"
            g.resultCheckAt=nil
            g.nextAttackAt=nil
        end
        if not g.stopped and g.target and g.phase=="attack" then
        ...
        if not g.stopped and g.phase=="attack" and g.resultCheckAt and C.Time>=g.resultCheckAt then
            ...
            if success then
                ...
                for _,sid in ipairs(g.infantry) do
                    if squadAlive(sid) then g.defenders[#g.defenders+1]=sid C.DetachedTarget[sid]=g.target end
                end
                g.infantry={}
                g.phase="defend" g.captured=true g.nextAttackAt=C.Time+NEU_BOT.NextAttackDelaySec
            else
...
local function advanceGroups()
    ...
    for gid,g in pairs(C.Groups) do
        ...
        if not g.stopped and f.neutralCount==0 and C.AttackUnlocked then
            if g.phase=="wait_neutral" then g.phase="defend" g.nextAttackAt=C.Time end
            if g.phase=="defend" and g.nextAttackAt and C.Time>=g.nextAttackAt then startGroupAttack(gid) end
        end
    end
end

local function processRecon()
    -- v1.5.2: recon no longer captures neutral flags. Neutral capture is infantry-only.
...
local function processInfantryReinforcement()
    if C.Time<C.NextInfantryReinforcementAt then return end
    C.NextInfantryReinforcementAt=C.Time+(NEU_BOT.InfantryReinforcementSec or 300)
    local f=flags()
    if f.neutralCount>0 then
        log("INFANTRY REINFORCEMENT time="..C.Time.." redirected=neutral-capture neutralLeft="..f.neutralCount)
        processNeutralCapture()
        return
    end
    for gid,g in pairs(C.Groups) do
        if not g.stopped then enqueueSpawn("infantry","5 minute infantry reinforcement",0,g.target,gid,"attack") end
    end
end
        ...
        if not C.DeadSquads[sid] and not squadAlive(sid) then
            ...
            if g and not g.stopped and g.phase=="attack" and g.target and flagOccupant(g.target)==BotApi.Instance.enemyTeam then
                if role=="infantry" or role=="infantry_detached" or role=="tank" or role=="tank80plus" then
                    g.stopped=true g.phase="stopped" g.nextAttackAt=nil g.resultCheckAt=nil
...
local function processEnemyRules()
    ...
    if s.aatSeenAt and s.aatStage==1 and C.Time>=s.aatSeenAt+NEU_BOT.AntiRadDelaySec then
        ...
        s.aatStage=2 s.aatCheckAt=C.Time+NEU_BOT.EnemyResponseCheckSec
...
function NEU_BOT_EnemyTagSeen(tag)
    tag=lower(tag)
    if tag=="aat" then
        C.EnemySignals.aatSeenAt=C.Time C.EnemySignals.aatDestroyedAt=nil C.EnemySignals.aatStage=1 C.EnemySignals.aatCheckAt=nil
    ...
    elseif tag=="aircraft" then
        ...
        if math.random(2)==1 then enqueueSpawn("duel_fighter","aircraft response 50/50",0,nil,nil,"support")
        else enqueueSpawn("aat","aircraft response 50/50",0,chooseOwnFlag(),nil,"support") end
...
local function onSecond()
    ...
    processInfantryReinforcement()
    processSpawn()
    if C.Time%30==0 then
        local f=flags()
        local parts={}
        for gid,g in pairs(C.Groups) do parts[#parts+1]="G"..gid.."="..tostring(g.target)..":"..tostring(g.phase) end
        local cap=0
        for sid,target in pairs(C.NeutralCaptureTarget) do if squadAlive(sid) and target then cap=cap+1 end end
...
function onGameStart()
    ...
    C.NextInfantryReinforcementAt=NEU_BOT.InfantryReinforcementSec or 300
    C.NextPatrolAt=0 C.NextInfantryOrderAt=0 C.NextNeutralCaptureAt=0
    C.ReconSquad=nil C.ReconTarget=nil
    C.NeutralCaptureTarget={} C.NeutralPending={}
    ...
    log("START v1.5.2 army="..tostring(BotApi.Instance.army).." playerId="..tostring(BotApi.Instance.playerId).." hostId="..tostring(BotApi.Instance.hostId).." units="..tostring(C.Units.count-1))
    log("RULE neutral-only-capture_infantry=ON capture-pair-size="..tostring(NEU_BOT.NeutralCaptureSquadSize or 2).." strict-neutral-first=ON infantry-5min=ON")
    startClock()
    processSpawn()
end
...
function onGameSpawn(args)
    ...
    if ticket.role=="recon" then
        ...
        -- Recon deliberately receives no neutral CaptureFlag order in v1.5.2.
    ...
    elseif ticket.role=="infantry" then
        ...
        if ticket.target and flagOccupant(ticket.target)==BotApi.Instance.enemyTeam then capture(sid,ticket.target) end
    ...
    elseif ticket.role=="defense_infantry" then
        ...
        if ticket.target and flagOccupant(ticket.target)==BotApi.Instance.team then capture(sid,ticket.target) end
    ...
    elseif ticket.role=="tank" or ticket.role=="tank80plus" then
        ...
        if t and flagOccupant(t)==BotApi.Instance.enemyTeam then deferCapture(sid,t,NEU_BOT.VehicleFollowDelaySec) end

    elseif ticket.role=="antiair" or ticket.role=="aat" then
        local own=ticket.target or chooseOwnFlag()
        if own and flagOccupant(own)==BotApi.Instance.team then capture(sid,own) end
