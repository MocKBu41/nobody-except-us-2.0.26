-- NEU BOT v1.5
-- Synced with bot-logic-editor (3).html.
-- Keeps v1.4.1 side-aware opening and independent assault groups.
...
local function availableTargets(includeEnemy)
    ...
    if includeEnemy or #list==0 then for _,n in ipairs(f.enemy) do list[#list+1]=n end end
    sortByOwnSide(list)
    return list
end
...
local function newGroup(target)
    ...
    C.Groups[id]={
        id=id,target=target,phase="opening",captured=false,
...
local function currentUsedTargets(exceptGid)
    local used={}
    for gid,g in pairs(C.Groups) do
        if gid~=exceptGid and g.target and g.phase~="defend" then used[g.target]=true end
    end
...
local function unlockAttackIfNeeded()
    ...
    if C.AttackUnlocked then
        ...
        for gid,g in pairs(C.Groups) do
            if g.phase=="opening" and g.target and flagOccupant(g.target)==BotApi.Instance.team then
                g.phase="defend"
...
local function processGroupOrders()
    ...
    for gid,g in pairs(C.Groups) do
        if g.target and g.phase~="defend" then
...
local function resolveAttackResults()
    for gid,g in pairs(C.Groups) do
        if g.phase=="attack" and g.resultCheckAt and C.Time>=g.resultCheckAt then
...
local function advanceGroups()
    for gid,g in pairs(C.Groups) do
        ...
        elseif g.phase=="defend" and C.AttackUnlocked and g.nextAttackAt and C.Time>=g.nextAttackAt then
    ...
    for sid,role in pairs(C.SquadRole) do
        if not C.DeadSquads[sid] and not squadAlive(sid) then
            ...
            if role=="tank" or role=="tank80plus" then
                local target=gid and C.Groups[gid] and C.Groups[gid].target or nil
                enqueueSpawn("tank80plus","tank destroyed replacement",0,target,gid,"attack")
                enqueueSpawn("infantry","tank destroyed infantry support",0,target,gid,"attack")
                enqueueSpawn("aat","tank destroyed random AA",0,chooseOwnFlag(),gid,"support")
                enqueueSpawn("aircraftlight","tank destroyed air support",0,target,gid,"support")
            end
        end
    end
end
...
local function onSecond()
    ...
    if C.Time%30==0 then
        ...
        log("TIME="..C.Time.." SIDE="..tostring(C.TeamEdge).." ATTACK="..tostring(C.AttackUnlocked).." FLAGS="..f.mineCount.."/"..f.enemyCount.." GROUPS "..table.concat(parts," "))
    end
end
...
function onGameStart()
    ...
    log("START v1.5 army="..tostring(BotApi.Instance.army).." playerId="..tostring(BotApi.Instance.playerId).." hostId="..tostring(BotApi.Instance.hostId).." units="..tostring(C.Units.count-1))
    startClock()
    processSpawn()
end
