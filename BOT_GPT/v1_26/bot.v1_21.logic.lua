...
function onGameStart()
 ...
 local host=tonumber(BotApi.Instance.hostId) or 1 math.randomseed(os.time()*host) local mapOk=N.MAP and N.MAP:load() or false queueOpening() N.log('START v1.21 map='..tostring(mapOk)) N.log('RULE BTG ASSEMBLY->APPROACH->ASSAULT; FAILED=>REGROUP+NEW ROUTE') N.log('RULE OWN SCENE SCAN=ON; SHARED RANDOM CORRIDOR=ON; ENEMY DIAGNOSTICS=ON') startClock() N.processSpawn()
end
