...
local function initTelemetry()
    TF.ready = false; TF.bytes = 0; TF.last = -999
    ...
    if f then
        local s = '{"type":"session","v":"2.0.4","team":' .. q(BotApi.Instance.team) .. ',"enemy":' .. q(BotApi.Instance.enemyTeam) .. ',"logic":"BOT_LOGIC_CURRENT","geometry":"disabled-until-runtime-map-id"}'
...
function N.log(m)
    ...
    if TF.ready and (x:find("STATE ",1,true) or x:find("TARGET ",1,true) or x:find("LOST ",1,true) or x:find("SPAWN ",1,true) or x:find("ARRIVED ",1,true) or x:find("HELI ",1,true) or x:find("OPENING ",1,true) or x:find("POINTSTART ",1,true) or x:find("ORDER ",1,true)) then
        append('{"type":"event","v":"2.0.4","t":' .. tostring(C.Time or 0) .. ',"team":' .. q(BotApi.Instance.team) .. ',"m":' .. q(x) .. '}')
    end
end
local function snapshot()
    ...
    table.sort(squads)
    append('{"type":"snapshot","v":"2.0.4","t":' .. tostring(C.Time) .. ',"state":' .. q(C.State) .. ',"bp":' .. tostring(C.Economy.points) .. ',"mine":' .. f.mineCount .. ',"enemy":' .. f.enemyCount .. ',"neutral":' .. f.neutralCount .. ',"squads":[' .. table.concat(squads, ",") .. ']}')
end
...
local function processOpening()
    if C.State ~= "OPENING" or C.MainOpeningFrozen then return end
...
function onGameStart()
    ...
    N.log("START BOT 2.0.4 team=" .. tostring(BotApi.Instance.team) .. " BP=" .. tostring(C.Economy.points))
