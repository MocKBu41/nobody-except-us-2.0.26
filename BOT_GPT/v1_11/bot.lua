-- NEU BOT v1.11
-- Based on v1.10.
-- Opening flags are captured ONLY by units tagged pointstart.
-- One pointstart squad per starting neutral flag.
-- If a pointstart squad dies before its assigned flag is captured,
-- that flag is treated as enemy-held even while BotApi still reports it neutral.
...
local C={
 ...
 NextTankReinforcementAt=0,NextInfantryReinforcementAt=0,NextPatrolAt=0,NextOrderAt=0,NextOpeningMaintenanceAt=0,NextPointStartCheckAt=0,
 PointStartTarget={},PointStartPending={},PointStartDone={},HostileNeutral={},StartPointList={},PointStartQueued=false,
 ReconSquad=nil,BalanceDefenseActive=false,
...
local function buildCandidates()
 local roles={"pointstart","recon","infantry","defense_infantry","antiair","tank","tank80plus","aircraftlight","aat","antirad","strike","duel_heli","duel_fighter"}
...
local function processSpawn()
 ...
 if ok then C.AwaitingArrival=ticket log("SPAWN OK role="..intent.role.." unit="..rec.unit.." group="..tostring(intent.groupId).." target="..tostring(intent.target).." maxSquad="..tostring(maxSize))
 else
 ...
 for id,g in pairs(C.Groups) do
  if not g.stopped and g.phase=="attack" and g.resultCheckAt and C.Time>=g.resultCheckAt then
   if g.target and flagOccupant(g.target)==BotApi.Instance.team then
    ...
    g.infantry={} g.phase="ready" g.resultCheckAt=nil g.nextAttackAt=C.Time+NEU_BOT.NextAttackDelaySec
   else
  ...
  if not C.DeadSquads[sid] and not squadAlive(sid) then
   ...
   if role=="pointstart" then
    ...
    if t then
     ...
     if flagOccupant(t)~=BotApi.Instance.team then
      C.HostileNeutral[t]=true
      log("POINTSTART LOST squad="..sid.." flag="..t.." -> HOSTILE evenIfNeutral="..tostring(isNeutral(t)))
     else
      C.PointStartDone[t]=true
     end
    end
   end
...
local function onSecond()
 ...
 if C.Time%30==0 then
  ...
  for id,g in pairs(C.Groups) do p[#p+1]="G"..id.."="..tostring(g.target)..":"..g.phase..":I"..aliveCount(g.infantry)..":T"..aliveCount(g.tanks) end
  log("TIME="..C.Time.." MAP="..tostring(MAP and MAP.mapKey).." FLAGS="..f.mineCount.."/"..f.enemyCount.." Nraw="..f.neutralCount.." Nstrategic="..strategicNeutralCount().." HOSTILE="..hostileNeutralActiveCount().." PS="..pointStartActiveCount().." "..table.concat(p," "))
 end
end
...
function onGameStart()
 ...
 C.AttackUnlocked=false C.AttackUnlockReason=nil C.NeutralsCleared=false C.NextTankReinforcementAt=NEU_BOT.TankReinforcementSec C.NextInfantryReinforcementAt=NEU_BOT.InfantryReinforcementSec C.NextPatrolAt=0 C.NextOrderAt=0 C.NextOpeningMaintenanceAt=0 C.NextPointStartCheckAt=0
 C.PointStartTarget={} C.PointStartPending={} C.PointStartDone={} C.HostileNeutral={} C.StartPointList={} C.PointStartQueued=false C.ReconSquad=nil C.BalanceDefenseActive=false
 ...
 log("START v1.11 army="..tostring(BotApi.Instance.army).." team="..tostring(BotApi.Instance.team).." units="..tostring((C.Units.count or 1)-1))
 log("RULE opening-capture=pointstart-ONLY one-squad-per-start-point=ON recon-neutral-capture=OFF")
...
function onGameSpawn(args)
 ...
 elseif ticket.role=="infantry" then
  local id=ticket.groupId if id and C.Groups[id] then C.Groups[id].infantry[#C.Groups[id].infantry+1]=sid if C.Groups[id].phase=="attack" and C.Groups[id].target then capture(sid,C.Groups[id].target,true) end end
