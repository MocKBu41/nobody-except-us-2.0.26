-- NEU BOT v1.21 telemetry exporter
-- Fixed telemetry location inside the NEU mod.
...
local function ensureTelemetryDir()
    -- Windows: mkdir is harmless when the directory already exists.
    if os and os.execute then
        pcall(os.execute, 'if not exist "'..TELEMETRY_DIR..'" mkdir "'..TELEMETRY_DIR..'"')
    end
end
...
function T.install(core)
    ...
    ensureTelemetryDir()
    local f=openTelemetry("w")
    if f then
        f:write('{"type":"session","version":"1.21","time":0,"path":'..q(TELEMETRY_PATH)..'}\n')
        f:flush()
        f:close()
        fileReady=true
    end
    ...
    if fileReady then
        baseLog('TELEMETRY v1.21 ACTIVE FIXED_PATH='..TELEMETRY_PATH)
    else
        baseLog('TELEMETRY v1.21 ERROR cannot create FIXED_PATH='..TELEMETRY_PATH)
        baseLog('TELEMETRY v1.21 HINT create folder manually: '..TELEMETRY_DIR)
    end
    return T
end
