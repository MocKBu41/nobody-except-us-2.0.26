...
local function flagOccupant(name)
    for _,flag in pairs(BotApi.Scene.Flags) do if flag.name==name then return flag.occupant end end
    return nil
end
...
local function chooseReconFlag()
    ...
    for _,name in ipairs(list) do
        if not C.ReconVisited[name] then C.ReconVisited[name]=true return name end
    end
    C.ReconVisited={}
    local name=list[1]
...
local function processDeferredOrders()
    for i=#C.DeferredOrders,1,-1 do
        ...
        if x.due<=C.Time then capture(x.squad,x.flag) table.remove(C.DeferredOrders,i) end
    end
end
...
local function orderAllAttack(target)
    ...
    for id,role in pairs(C.SquadRole) do
        if squadAlive(id) and role=="infantry" then
            giveRoleOrder(id,role,target)
            requestDisembark(id)
        elseif squadAlive(id) and role=="recon" then
            giveRoleOrder(id,role,target)
        end
    end
    for id,role in pairs(C.SquadRole) do
...
local function startAttack()
    ...
    if not target then
        C.State="DEFEND" C.CurrentAttackFlag=nil orderDefense()
...
local function processAttack()
    ...
    if flagOccupant(C.CurrentAttackFlag)==BotApi.Instance.team then
        ...
        log("ATTACK SUCCESS flag="..capturedFlag)
...
local function processTankReinforcement()
    ...
    enqueueSpawn("tank","5 minute tank reinforcement",0,C.CurrentAttackFlag)
...
local function processStrategicLoop()
    ...
    elseif C.State=="DEFEND" then
        local f=flags()
        if f.enemyCount>=f.mineCount then C.State="WAIT" C.NextAttackAt=C.Time+NEU_BOT.NextAttackDelaySec end
    end
end
...
local function processEnemyRules()
    ...
    elseif s.aatStage==2 and s.aatCheckAt and C.Time>=s.aatCheckAt then
        if s.aatDestroyedAt and s.aatDestroyedAt>=s.aatSeenAt then
            enqueueSpawn("duel_heli","aat destroyed",0,C.CurrentAttackFlag) s.aatStage=0
        else
            enqueueSpawn("antirad","aat survived",0,C.CurrentAttackFlag)
            enqueueSpawn("strike","aat survived",0,C.CurrentAttackFlag)
            s.aatStage=3 s.aatCheckAt=C.Time+NEU_BOT.EnemyResponseCheckSec
        end
    elseif s.aatStage==3 and s.aatCheckAt and C.Time>=s.aatCheckAt then
        if s.aatDestroyedAt and s.aatDestroyedAt>=s.aatSeenAt then
            enqueueSpawn("duel_heli","aat destroyed after strike",0,C.CurrentAttackFlag) s.aatStage=0
        else
...
function NEU_BOT_EnemyTagSeen(tag)
    tag=lower(tag)
    if tag=="aat" then
        C.EnemySignals.aatSeenAt=C.Time C.EnemySignals.aatDestroyedAt=nil C.EnemySignals.aatStage=1
...
function NEU_BOT_EnemyTagDestroyed(tag)
    tag=lower(tag)
    if tag=="aat" then C.EnemySignals.aatDestroyedAt=C.Time
    elseif tag=="aircraft" then C.EnemySignals.aircraftDestroyedAt=C.Time end
end
...
local function startClock()
    ...
    local function pulse()
        ...
        C.Timer=nil
        onSecond()
        if generation==C.TimerGeneration then C.Timer=BotApi.Events:SetQuantTimer(pulse,NEU_BOT.TickMs) end
    end
...
function onGameStart()
    ...
    enqueueSpawn("tank","opening tank",NEU_BOT.OpeningTankDelaySec,nil)

    log("START v1.2 army="..tostring(BotApi.Instance.army).." units="..tostring(C.Units.count-1))
    startClock()
    processSpawn()
end
