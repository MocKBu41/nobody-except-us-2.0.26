...
function N.enqueueSpawn(role,reason,delay,target,gid,mode)
 if role=='aircraftlight' then
  if C.Time<(C.NextAircraftAllowedAt or 0) then return false end
  for _,x in ipairs(C.SpawnIntents) do if x.role==role then return false end end
  if C.AwaitingArrival and C.AwaitingArrival.role==role then return false end
  C.NextAircraftAllowedAt=C.Time+math.max(360,NEU_BOT.AirSupportCooldownSec or 360)
  N.log('AIR COOLDOWN START next='..C.NextAircraftAllowedAt..' cd=360s')
 end
