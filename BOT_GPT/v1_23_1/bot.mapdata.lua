...
function M:load()
    ...
    local best=nil
    local requested=NEU_BOT and NEU_BOT.MapKey
    if type(requested)=="string" and requested~="" then
        local selected=root.maps[requested]
        if not selected then log("MAP KEY NOT FOUND "..requested) return false end
        local ratio,hit,extra,mapCount=matchScore(runtimeNames,runtimeSet,selected)
        best={key=requested,map=selected,ratio=ratio,hit=hit,extra=extra,mapCount=mapCount}
        log("MAP EXPLICIT "..requested)
    else
        -- Flag names are reused across maps. Never guess between equally covering maps.
        local matches={}
        for key,map in pairs(root.maps) do
            local ratio,hit,extra,mapCount=matchScore(runtimeNames,runtimeSet,map)
            if #runtimeNames>0 and hit==#runtimeNames then
                matches[#matches+1]={key=key,map=map,ratio=ratio,hit=hit,extra=extra,mapCount=mapCount}
            end
        end
        if #matches~=1 then
            log("MAP AMBIGUOUS candidates="..#matches.."; set NEU_BOT.MapKey; geometry disabled")
            return false
        end
        best=matches[1]
    end
